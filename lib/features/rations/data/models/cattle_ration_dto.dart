...
class CattleRationDto {
  final int id;
  final int? cattleId; // ✅ nullable, потому что бек реально присылает null
  final String? tagNumber;
  final String? name;
  final String? animalCategory;
  final String? productionState;
  final String? breedType;
  final double? currentWeight; // ✅ nullable (у тебя в логах null)
  ...
  factory CattleRationDto.fromJson(Map<String, dynamic> json) {
    ...
    return CattleRationDto(
      ...
      cattleId: (json['cattleId'] as num?)?.toInt(), // ✅ без падения
      ...
      currentWeight: (json['currentWeight'] as num?)?.toDouble(), // ✅ nullable
      ...
      items: rawItems
          ...
          .map(CattleRationItemDto.fromJson)
          .toList(),
    );
  }
}
